      ...
      <div class="w-2/3">
        <div class="grid grid-cols-3 gap-5">
          <div v-for="(item, index) in categories.filtered" :key="index" class="pb-5">
            <div
              class="pb-80 relative group overflow-hidden cursor-pointer"
              @click="() => goToProduct(item.id)"
            >
              <div
                class="absolute z-10 cursor-pointer bottom-0 left-0 bg-terracota-500 transition-all duration-200 transform group-hover:translate-y-0 translate-y-16 w-full p-2 text-center text-md font-bold text-white"
              >
                <div
                  v-if="!item.stock"
                  class="border-3 border-dashed p-2 border-white flex justify-center items-center space-x-3 z-10"
                >
                ...
                <div
                  v-else
                  class="border-3 border-dashed p-2 border-white flex justify-center items-center space-x-3"
                >
                  <Icon icon="material-symbols:favorite-outline" class="text-2xl" />
                </div>
              </div>
              <img
                :src="item.product"
                class="text-shadow-md object-cover group-hover:opacity-0 opacity-100 transition-all duration-500 absolute w-full h-full"
              />
              <!-- TODO: change for second image -->
              <img
                :src="item.pictures[1].picture"
                class="text-shadow-md object-cover group-hover:opacity-100 block opacity-0 transition-all duration-500 absolute w-full h-full"
              />
              <div class="absolute top-4 right-4 text-shadow-md">
                <span
                  v-if="item.stock"
                  class="inline-flex items-center rounded-xl bg-gray-50 px-3 py-2 !rounded-full text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-200"
                >
                  OUT OF STOCK
                </span>
              </div>
